...
|-------------------|--------|------:|---------|
| IT Service Management (ITSM) | `it-service-management` | 70 | [README](src/tools/it-service-management/README.md) |
| Now Platform (core) | `now-platform` | 89 | [README](src/tools/now-platform/README.md) |
| Source-to-Pay Operations | `source-to-pay-operations` | 58 | [README](src/tools/source-to-pay-operations/README.md) |
| ServiceNow Platform (CMDB, Knowledge, Interaction, Skills) | `servicenow-platform` | 28 | [README](src/tools/servicenow-platform/README.md) |
| Application Development | `application-development` | 50 | [README](src/tools/application-development/README.md) |
| Platform User Interface | `platform-user-interface` | 29 | [README](src/tools/platform-user-interface/README.md) |
...
| Platform Security | `platform-security` | 25 | [README](src/tools/platform-security/README.md) |
...
_58 tool modules · 418 tools across 15 ServiceNow module folders._
...
If `.env` already has at least one instance configured, `npm run setup`'s first question is "Connect using this configuration, or manage instances (add/update/delete)?" — it prints what's already set (mode, instance names, URLs, auth methods) and defaults to "connect", which leaves `.env` untouched (beyond optionally picking a new `SERVICENOW_DEFAULT_INSTANCE` if you have more than one) and just restarts the server so your client reconnects with it. Answer "manage" to get a menu — add a new instance, update or delete an existing one, or reconfigure everything from scratch — and adding/updating/deleting only ever touches the instance you picked; the others are left exactly as they were. Moving from a single instance to a second one migrates the first instance's existing credentials to a named slot automatically, so nothing needs re-entering.
...
An MCP client that supports elicitation (Claude Code/Desktop) is asked which instance to use as soon as the server starts (right after the connection handshake completes) — not just with multiple instances configured; even a single-instance setup gets the prompt, with an "Add a new instance..." option alongside whatever's already configured. Picking that doesn't collect credentials in the prompt itself — it points you at `npm run setup` (which never echoes/logs secrets and encrypts them at rest via dotenvx) and keeps the current session on whatever instance was already active, since a running server can't pick up new instances until it's restarted. `SERVICENOW_DEFAULT_INSTANCE` picks the fallback for clients that don't support elicitation, or if the prompt is declined. Switch instances mid-conversation with `sn_instance_switch`; list what's configured (and which is active) with `sn_instance_list`. Both work in `debug` mode — they're session/config metadata, not a ServiceNow write.
