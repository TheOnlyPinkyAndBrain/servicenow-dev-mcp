...
| `sn_ui_policy_action_create` | develop | Create a new UI Policy Action (sys_ui_policy_action) and link it to its parent UI Policy. Routes through the background-script engine rather than the plain Table API, because this instance's 'ui_policy' field isn't settable via a direct REST write. |
| `sn_ui_policy_action_delete` | develop | Delete a UI Policy Action (sys_ui_policy_action) by sys_id |
| `sn_ui_policy_action_get` | both | Get a single UI Policy Action (sys_ui_policy_action) by sys_id |
| `sn_ui_policy_action_update` | develop | Update an existing UI Policy Action (sys_ui_policy_action), including reassigning it to a different UI Policy via 'ui_policy'. Routes through the background-script engine for the same reason as sn_ui_policy_action_create. |
