...
export function registerUniversalRequestTools(
  ...
  server.tool(
    ...
    {
      opened_for: z.string().optional().describe("Filter by opened_for user sys_id (the employee the request is for)"),
    ...
    READ,
    async ({ opened_for, state, assignment_group, query, limit, offset }) => {
      try {
        ...
        const result = await client.query("universal_request", {
          ...
          sysparm_fields: "sys_id,number,short_description,state,opened_for,opened_by,assignment_group,assigned_to,sys_created_on,sys_updated_on",
          sysparm_limit: limit,
