...
| `sn_catalog_ui_policy_action_get` | both | Get a single Catalog UI Policy Action by sys_id |
| `sn_catalog_ui_policy_action_update` | develop | Update an existing Catalog UI Policy Action (catalog_ui_policy_action), including reassigning it via 'ui_policy'. Routes through the background-script engine for the same reason as sn_catalog_ui_policy_action_create. |
