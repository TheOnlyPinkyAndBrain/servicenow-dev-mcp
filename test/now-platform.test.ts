...
const REGISTRARS = [
  registerTableTools, registerSystemTools, registerSchemaTools, registerBatchTools,
...
function register(mode: Mode) {
  ...
  // registerExecuteTools takes two extra required args (enableScriptExecute,
  // instanceUrl) on top of the standard Registrar signature, since
  // sn_script_execute needs an explicit opt-in beyond just mode=develop.
  registerExecuteTools(server as never, client as never, mode, true, "https://dev.example.com");
  return { tools, client };
}
