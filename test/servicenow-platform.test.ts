...
function mockClient() {
  ...
  const client = {
    ...
    executeBackgroundScript: async (script: string, scope?: string, elevate?: boolean) => {
      calls.push(["execScript", script, scope, elevate]);
      return { success: true, output: JSON.stringify({ sys_id: "new1" }) };
    },
  };
  return client;
}
...
describe("relocated config tools land in the right modules", () => {
  it("ACL tools query sys_security_acl", async () => {
    ...
    // sn_acl_create routes through the background-script engine with
    // elevation forced on (ServiceNow refuses ACL writes without an active
    // security_admin session), not the plain Table API.
    await tools.get("sn_acl_create")!.handler({ data: { name: "x" } });
    const execCall = client.calls.find((c) => c[0] === "execScript");
    expect(execCall?.[1]).toContain("sys_security_acl");
    expect(execCall?.[3]).toBe(true);
  });
